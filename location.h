...
int increaseCaseCount(string location, string diseaseName,  int cases)
{
    ...
    for (int i = 0; i < diseases.size(); i++){
        Disease disease = diseases[i];
        if (disease.locationName == location && disease.diseaseName == diseaseName){
            disease.cases += cases;
            newCases = disease.cases;
        }
    }
